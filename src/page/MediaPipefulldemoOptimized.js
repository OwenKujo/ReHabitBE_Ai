...
import MediaPipeNavigation from '../components/MediaPipeNavigation';

function MediaPipeOptimized() {
  const { lang } = useLang();
  ...
  const {
    ...
    initializeFaceMesh,
    cleanup: faceCleanup
  } = useSmoothFaceDetection(videoRef, canvasRef);
  ...
  const currentPerformance = mode === 'pose' ? posePerformance : facePerformance;
...
export default MediaPipeOptimized; 