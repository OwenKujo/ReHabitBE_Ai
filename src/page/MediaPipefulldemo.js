...
function playBeep(frequency = 800, duration = 300, volume = 0.3) {
  ...
  const [countdown, setCountdown] = useState(10); // countdonw for Pose
  const [heldTime, setHeldTime] = useState(0); // heldtime for Pose
...
const targetReps = 5;
const [faceSet, setFaceSet] = useState(1);
const faceTotalSets = 5;
const faceHoldTimePerSet = 10;
const faceRestTimePerSet = 5;
// Removed duplicate declaration of phase and setPhase
const [facePhase, setFacePhase] = useState("countdown");
...
const [faceCorrectTime, setFaceCorrectTime] = useState(0);
...
const loadMediaPipeScriptsFace = () => {
  ...
  const onFaceResults = (results) => {
    ...
    if (results.multiFaceLandmarks?.length > 0) {
      ...
      if (mode === "face" && facePhase === "challenge") {
        if (pitch > pitchThreshold) {
          ...
          holdStartTimeRef.current = null;
          setHoldTime(0);
          console.log(`Pitch: ${pitch} , PitchThreshold: ${pitchThreshold}`);
    ...
    if (mode === "pose") {
      ...
      intervalRef.current = setInterval(() => {
        setCountdown((prev) => {
          if (prev <= 1) {
            ...
            startChallenge(); // เริ่ม challenge ของ pose
            return 0;
          }
          return prev - 1;
        });
      ...
      intervalRef.current = setInterval(() => {
        setFaceCountdown((prev) => {
          if (prev <= 1) {
            clearInterval(intervalRef.current);
            startFaceChallenge(); // ฟังก์ชันที่คุณจะใช้เริ่ม challenge ใน face mode
            return 0;
          }
          return prev - 1;
        });
...
setFaceCountdown(10);
  ...
  useEffect(() => {
    if (mode === "face" && repCount >= targetReps) {
      ...
      if (mode === "face" && repCount >= targetReps) {
        if (faceSet < faceTotalSets) {
          setFaceSet(faceSet + 1);
          setRepCount(0);  // reset reps
          startRest();     // rest between face sets
        } else {
          setPhase("finished");
  ...
  useEffect(() => {
    if (mode === "face" && facePhase === "countdown" && faceCountdown === 10 && !intervalRef.current) {
  ...
  if (mode === "face") {
    // Challenge phase UI
    if (["countdown", "challenge", "rest", "finished", "showfinal"].includes(facePhase)) {
      return (
          ...
          <div style={{
            ...
            {facePhase === "countdown" && (
              <>
                ...
                <div style={{ fontSize: 20, color: "#1976d2", fontWeight: "bold", marginBottom: 8 }}>Countdown: {faceCountdown} s</div>
              </>
            )}
            {facePhase === "challenge" && (
              <>
                <div style={{ fontSize: 20, color: "#1976d2", fontWeight: "bold", marginBottom: 8 }}>Set {faceCurrentSet} / {totalSets}</div>
                <div style={{ fontSize: 20, color: "#1976d2", fontWeight: "bold", marginBottom: 8 }}>Countdown: {faceChallengeCountdown} s</div>
                <div style={{ marginBottom: 8 }}>Hold your head back for 10 seconds!</div>
              </>
            )}
            {facePhase === "rest" && (
              <>
                <div style={{ fontSize: 20, color: "#FFA500", fontWeight: "bold", marginBottom: 8 }}>Rest Time: {faceRestCountdown} s</div>
                <div style={{ marginBottom: 8 }}>Next: Set {faceCurrentSet} / {totalSets}</div>
              </>
            )}
            {facePhase === "finished" && (
              <>
